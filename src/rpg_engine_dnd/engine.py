...
class SimulationEngine:
    ...
    def _next_event(self, command: Command, *, kind: str, entity_id: str | None, payload: dict[str, object] | None = None) -> Event:
        self.world.revision += 1
        return Event(sequence=self.world.revision, command_id=command.command_id, kind=kind, entity_id=entity_id, payload={} if payload is None else payload)
    ...
    def _create(self, command: CreateEntity) -> Event:
        ...
        self.world.entities[command.entity_id] = Entity(entity_id=command.entity_id, components=deepcopy(command.components))
        return self._next_event(command, kind="entity.created", entity_id=command.entity_id, payload={"components": deepcopy(command.components)})
    ...
    def _delete(self, command: DeleteEntity) -> Event:
        ...
        return self._next_event(command, kind="entity.deleted", entity_id=command.entity_id, payload=payload)

    def _set_component(self, command: SetComponent) -> Event:
        entity = self.world.entity(command.entity_id)
        entity.components[command.component] = deepcopy(command.value)
        return self._next_event(command, kind="component.set", entity_id=command.entity_id, payload={"component": command.component, "value": deepcopy(command.value)})

    def _patch_component(self, command: PatchComponent) -> Event:
        entity = self.world.entity(command.entity_id)
        if command.component not in entity.components:
            raise KeyError(f"unknown component {command.component!r} on entity {command.entity_id!r}")
        entity.components[command.component].update(deepcopy(command.patch))
        return self._next_event(command, kind="component.patched", entity_id=command.entity_id, payload={"component": command.component, "patch": deepcopy(command.patch)})
    ...
    def _remove_component(self, command: RemoveComponent) -> Event:
        ...
        try:
            ...
            raise KeyError(f"unknown component {command.component!r} on entity {command.entity_id!r}") from exc
        return self._next_event(command, kind="component.removed", entity_id=command.entity_id, payload={"component": command.component, "value": deepcopy(value)})
