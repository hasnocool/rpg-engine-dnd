...
<title>rpg-engine-dnd Creator Studio</title>
<style>
...
<body>
<header><strong>rpg-engine-dnd v3</strong><span class="small">Creator Studio · selectable projects + portable content library</span></header>
<main>
<aside>
<h3>Content library</h3>
<label>Bundled project</label>
<select id="libraryProject"><option value="shattered-beacon">The Shattered Beacon</option></select>
<button id="loadLibrary" class="primary">Load selected project into Studio</button>
<div id="librarySummary" class="small"></div>
<label>Project content</label>
<select id="libraryItem"><option value="">Load a project first</option></select>
<button id="loadItem">Load selected item into editor</button>
<hr>
<h3>Studio project</h3>
<label>Project ID</label><input id="project" value="demo-project">
...
<button id="importItem">Import selected file</button>
<p class="small">All items are SHA-256 checked and the server revalidates their typed Studio models on save.</p>
<hr>
<details>
<summary>Runtime campaign controls</summary>
<label>Campaign ID</label><input id="campaign" value="demo">
<label>Owner ID</label><input id="owner" value="local-owner">
<button id="create">Create runtime campaign</button>
...
<h3>Runtime world</h3><pre id="world">No campaign loaded.</pre>
</section>
</main>
<script>
const $ = id => document.getElementById(id);
const state = { nodes:[], edges:[], selected:[], drag:null, items:[], selectedItemKey:'' };
...
function refreshItemSelector(preferred=''){const select=$('libraryItem');const current=preferred||select.value;select.innerHTML='';for(const item of state.items){const option=document.createElement('option');option.value=itemKey(item);option.textContent=`${item.kind} · ${item.item_id}`;select.appendChild(option);}if(current&&state.items.some(i=>itemKey(i)===current))select.value=current;state.selectedItemKey=select.value;updateItemEditor();}
function updateItemEditor(){const item=selectedItem();$('itemEditor').value=item?JSON.stringify(item.content,null,2):'{}';}
$('libraryItem').onchange=()=>{state.selectedItemKey=$('libraryItem').value;updateItemEditor();};

const BUNDLED_PROJECTS={
 'shattered-beacon':{
  project_id:'example-shattered-beacon',name:'The Shattered Beacon',description:'Original coastal mystery starter campaign.',
  items:[
...
async function loadBundledProject(key){const source=BUNDLED_PROJECTS[key];if(!source)throw new Error('Unknown bundled Studio project.');state.items=[];for(const raw of source.items)state.items.push(await envelope(raw.kind,raw.item_id,raw.content));state.items.sort((a,b)=>itemKey(a).localeCompare(itemKey(b)));const map=state.items.find(i=>i.kind==='map');state.nodes=map?structuredClone(map.content.nodes||[]):[];state.edges=map?structuredClone(map.content.edges||[]):[];$('project').value=source.project_id;$('mapItem').value=map?map.item_id:'world-map';refreshItemSelector(map?itemKey(map):'');render();const counts={};for(const item of state.items)counts[item.kind]=(counts[item.kind]||0)+1;$('librarySummary').innerHTML=`<strong>${source.name}</strong><br>${source.description}<br>`+Object.entries(counts).map(([k,v])=>`<span class="badge">${k}: ${v}</span>`).join('');const document={description:source.description,map:{nodes:state.nodes,edges:state.edges},items:state.items};await request('/v3/studio/projects',{method:'POST',headers:headers(),body:JSON.stringify({project_id:source.project_id,name:source.name,document})});}
...
async function refresh(){try{$('world').textContent=JSON.stringify(await request(`/v3/campaigns/${$('campaign').value}`,{headers:headers()}),null,2);}catch(e){$('world').textContent=String(e);}}
