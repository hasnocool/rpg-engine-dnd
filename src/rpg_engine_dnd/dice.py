...
class DiceStreams:
    ...
    Each stream receives its own `random.Random` instance. This prevents an unrelated
    subsystem from perturbing another subsystem's future rolls merely by consuming RNG.
    """
    ...
    @staticmethod
    def _normalize_seed(seed: int | str | bytes) -> bytes:
    ...
    def roll(self, expression: str, *, stream: str = "default") -> DiceRoll:
        ...
        if count > 1_000:
            raise ValueError("dice count exceeds safety limit")
        if sides > 1_000_000:
            raise ValueError("die size exceeds safety limit")

        rng = self.stream(stream)
